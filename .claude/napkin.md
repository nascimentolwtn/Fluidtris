...
## Done
- **[2026-07-26] Feature: maze route-assist (multiple paths + show-route + recalculate)** — `MazeGenerator.generate()` gained a braid pass (`GameConstants.MAZE_BRAID_PERCENT`, default `0.35f`) that opens a fraction of dead ends into loops, so mazes can have more than one valid route (perfect-maze default of `0f` preserved for backward compat). New `MazeSolver.kt` does a pure BFS shortest-path search. `GameEngine` exposes `toggleMazeRoute()`/`mazeRouteCells()`; the route recomputes unconditionally after every successful `attemptMazeMove()` while visible, so it stays current if the player deviates onto a different path. `FluidTetrisView` reuses the existing side-button strips (now backed by shared `GameConstants.SIDE_BUTTON_TOP`/`SIDE_BUTTON_BOTTOM_MARGIN`, extracted into `drawSideButtons()`/`isSideButtonTouch()` helpers) for a "route"/"hide" toggle, rendering the hint as dots in `drawMaze()`. Implemented per plan at `.claude/plans/maze-route-assist.md`. Fixed during review: the maze swipe-gesture touch anchor (`mazeTouchRefX/Y`) now refreshes on every touch-down while a maze is active, not only the swipe fallback — previously a touch that landed on a button and then dragged into the grid without lifting could compute its first move against a stale anchor.
...
- **[2026-06-17] Feature: unified level multiplier affects gravity** — `getLevelMultiplier()` returns `(1 + (level - 1) * LEVEL_DIFFICULTY_FACTOR).coerceAtMost(MAX_LEVEL_MULTIPLIER)` where level = score / 300 + 1. Each 300 points advances to next level. Applies to gravity scaling: pieces fall progressively faster at higher levels. Constants `LEVEL_DIFFICULTY_FACTOR=0.3f` and `MAX_LEVEL_MULTIPLIER=3f` in `GameConstants`. Public `getLevel()` getter. Unit test `GameEngineLevelTest` covers level progression, multiplier scaling, and gravity increase with 4 tests. All 161 tests pass.
- **[2026-06-17] Feature: Level display and level-up sound** — "Level: X" label drawn in top-left corner (line 191 FluidTetrisView). Level starts at 1 and increases every 300 points (level = score / 300 + 1). `onLevelUp()` callback fired in `checkLines()` wired to `soundManager.playLevelUpSound()`. `SoundManager.playLevelUpSound()` plays `R.raw.level_up_sound`. Feature triggered when level increases during line clear scoring. All 170 tests pass.
- **[2026-06-15] Feature: gravity increases with score (each 300 points / level)** — formula: `GRAVITY * getLevelMultiplier()` where level multiplier scales by `LEVEL_DIFFICULTY_FACTOR` per level. Gravity scales with level progression: each 300 points increases falling speed, making the game progressively harder. Implemented via `getLevelMultiplier()` in `GameEngine.update()` line 100. All tests pass.
