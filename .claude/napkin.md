...
## Backlog
1. **[2026-06-17] Fix: Next button freezes other falling pieces** — `update()` method was not applying gravity to `otherFallingPieces`. Added loop to apply scaled gravity (based on level multiplier) and wall clamping to each piece in `otherFallingPieces` each frame. Now pieces continue falling naturally when spawned via Next button. All tests pass (same 2 pre-existing GameNoInteractionTest flaky tests remain).
